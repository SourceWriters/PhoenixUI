package net.sourcewriters.minecraft.phoenixui.api.message;
...
public final class Component extends SendableComponent {
    ...
    public Component(final String message) {
        this(IPhoenixUIApi.get(), message);
    }

    public Component(final IPhoenixUIApi api, final String message) {
        this.api = Objects.requireNonNull(api, "IPhoenixUIApi can't be null!");
    ...
    public Component hoverItem(final ItemStack itemStack) {
        if (itemStack == null || itemStack.getType().isAir()) {
            return this;
        }
        return hover(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new Item(itemStack.getType().getKey().toString(), itemStack.getAmount(),
            ItemTag.ofNbt(NbtProvider.itemStackToNbt(itemStack).toString()))));
    }
    ...
    public Component hoverText(final String messageId, final Key... placeholders) {
        if (message == null) {
            return this;
        }
        return hoverText(api.getRawMessage(messageId, placeholders));
    }

    public Component hoverText(final String language, final String messageId, final Key... placeholders) {
        if (message == null) {
            return this;
        }
        return hoverText(api.getRawMessage(language, messageId, placeholders));
    }
